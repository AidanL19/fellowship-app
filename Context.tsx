...
export const SpendingGoalsProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  const [transactionSubmission, setTransactionSubmission] =
    useState<string>("");
  ...
  const updateSpending = (results: any) => {
    setSpendingList(
      results.map((item: { spending_list: string[] }) => item.spending_list)
    );
    const accumulatedAmount: any = results.reduce(
  ...
  const updateLimitGoals = (results: any) => {
    setLimitGoalsList(
      results.map(
        (item: { limit_goals_list: string[] }) => item.limit_goals_list
      )
    );
  };
  ...
  const updateCutDownGoals = (results: any) => {
    setCutDownGoalsList(
      results.map(
        (item: { cut_down_goals_list: string[] }) => item.cut_down_goals_list
      )
    );
  };
  ...
  const updateGoalsToRemove = (results: any) => {
    setGoalsToRemove(
      results.map(
        (item: { removed_goals_list: string[] }) => item.removed_goals_list
      )
    );
  };

  const updateVisualizationList = (results: any, ss: boolean) => {
    if (ss) {
      setVisualizationList(
        results.map((item: { section_list: string[] }) => item.section_list)
      );
    } else {
      setVisualizationList(
        results.map(
          (item: { subsection_list: string[] }) => item.subsection_list
        )
      );
    }
  };
